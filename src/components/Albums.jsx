...
const Albums = (props) => {
  const handleSelection = (album, e) => {
    props.handleAlbumsStateChange({
      album,
  ...
  return (
    <Fragment>
        ...
        <div className="row">
              ...
              return (
                ...
                >
                  <input
                    ...
                    id={album._id}
                    name={album.name}
                    onChange={(e) => handleSelection(album, e)}
                    value={album.selected}
                    defaultChecked={album.selected}
                  />
                  <label htmlFor={album._id} className="user-select-none">
                    {album.name}
                  </label>
                </div>
              );
          ...
          <div className="custom-height row mx-auto col-lg-3 col-12 py-2 mt-3 my-lg-0">
            <div className="col-lg-12 col-6 text-white text-center text-info">
              Count: {props.albumsCount}
            </div>
            <div className="col-lg-12 col-6 text-white text-center text-info">
              Price: {props.albumsPrice}
            </div>
          </div>
        </div>
      </div>
      <div className="container my-5">
        <div className="row justify-content-around mx-auto col-12">
          <Link to="/singers" className="col-lg-3 col-6 text-center">
            <svg
	...
	l144.1-89.788L326.239,217.62z"
          ...
          <Link
            to={
              props.albums.some((album) => album.selected)
                ? "/songs"
                : "/albums"
            }
            onClick={() => {
              if (!props.albums.some((album) => album.selected)) {
                toast.error("Please select at least one album!");
              }
              // // //
              // for (let index = 0; index < props.albums.length; index++) {
              //   props.handleAlbumsStateChange({
              //     album: props.albums[index],
              //     checked: props.albums[index].selected,
              //   });
              // }
              // //
            }}
