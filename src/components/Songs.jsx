...
const Songs = (props) => {
  const handleSelection = (song, e) => {
    props.handleSongsStateChange({
      song,
  ...
  return (
    <Fragment>
        ...
        <div className="row">
              ...
              return (
                ...
                >
                  <input
                    ...
                    id={song._id}
                    name={song.name}
                    onChange={(e) => handleSelection(song, e)}
                    value={song.selected}
                    defaultChecked={song.selected}
                  />
                  <label htmlFor={song._id} className="user-select-none">
                    {song.name}
                  </label>
                </div>
              );
          ...
          <div className="custom-height row mx-auto col-lg-3 col-12 py-2 mt-3 my-lg-0">
            <div className="col-lg-12 col-6 text-white text-center text-info">
              Count: {props.songsCount}
            </div>
            <div className="col-lg-12 col-6 text-white text-center text-info">
              Price: {props.songsPrice}
            </div>
          </div>
        </div>
      </div>
      <div className="container my-5">
        <div className="row justify-content-around mx-auto col-12">
          <Link to="/albums" className="col-lg-3 col-6 text-center">
            <svg
	...
	l144.1-89.788L326.239,217.62z"
          ...
          <Link
            to={
              props.songs.some((song) => song.selected) ? "/submit" : "/songs"
            }
            onClick={() => {
              if (!props.songs.some((song) => song.selected)) {
